...
public class PlayerController : MonoBehaviour
{
    ...
    {
        if (!die)
        {       //stop doing anything when player dead
            if (play && !isCannonFiring)
            {       //only moving when play varible is true and not fire by the Big Cannon
                transform.Translate(new Vector3(speed, 0, 0));      //moving the player with speed 
            }
            if (isUsingJetPack && isJumpHold)
            {       //if player got JetPack mode, the Jump button will be used to raise the player up
            ...
            {
//-----------------------------------------------------------------------------------------------------------------------
                Jump();  //CÓDIGO EDITADO: se cambio que en vez de que cayera, el pinguino saltara al detectar un vacio.
//-----------------------------------------------------------------------------------------------------------------------
    ...
    public void Attack()
    {
        if (!die && !isCannonFiring)
        {
            ...
            {       //only allow throw the bullet when the amount of bullet greater then zero
                GameManager.Bullets--;
...
//----------------------------------------------------------+--------------------------------------------
//CÓDIGO EDITADO: para realizar una acción al detectar las siguientes etiquetas
